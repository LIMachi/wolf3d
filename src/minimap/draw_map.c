...
static inline void	draw_line_0(t_glfw_window *win, t_vec v[4], int e, uint32_t color)
{
	if (v[2].x > v[2].y)
		while (v[0].x != v[1].x)
		{
...
t_glfw_window	*draw_map(t_glfw_window *win, t_header *map)
{
	...
	while (++x < map->width)
	{
		y = -1;
		while (++y < map->height)
			draw_square(win, (t_vec){(uint32_t)(x * sx), (uint32_t)(y * sy)}, (t_vec){(uint32_t)sx, (uint32_t)sy}, map->map[x + y * map->width] ? 0 : 0xFFFFFF);
	}
	t_vector	look = rotate_2d((t_vector){.x = 0, .y = -10}, (double)map->look / 100.0);
	draw_square(win, (t_vec){.x = map->startx * sx - 2, .y = map->starty * sy - 2}, (t_vec){.x = 5, .y = 5}, 0xFF00);
	draw_line(win, (t_vec){.x = map->startx * sx, .y = map->starty * sy}, (t_vec){.x = look.x + map->startx * sx, .y = look.y + map->starty * sy}, 0xFF0000);
	return (win);
}
